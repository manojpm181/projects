        ...
        try {
            switch (request.params.name) {
                case 'get_stock_price_history': {
                    const { ticker, start_date, end_date, limit = 30 } = request.params.arguments;
                    // Get company ID
            ...
            `)
                    ...
                    const screened = (companies || []).filter(company => {
                        ...
                        if (!latestReport)
                            return false;
                        if (args.min_revenue && latestReport.revenue < args.min_revenue)
                            return false;
                        if (args.min_eps && latestReport.eps && latestReport.eps < args.min_eps)
                            return false;
                        if (args.min_gross_margin && latestReport.gross_margin && latestReport.gross_margin < args.min_gross_margin)
                            return false;
                        if (args.max_debt_to_equity && latestReport.debt_to_equity && latestReport.debt_to_equity > args.max_debt_to_equity)
                            return false;
                        if (args.sector && company.sector !== args.sector)
                            return false;
                        if (args.min_market_cap && company.market_cap < args.min_market_cap)
                            return false;
                        if (args.max_market_cap && company.market_cap > args.max_market_cap)
                            return false;
                        return true;
                    });
                    return {
