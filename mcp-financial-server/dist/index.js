...
async function main() {
    ...
    if (!dbConnected)
        process.exit(1);
    const server = new Server({ name: 'financial-data-server', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    if (mode === 'sse') {
        ...
        httpServer.listen(port, () => logger.info(`SSE running on http://localhost:${port}/message`));
    }
    else {
